...
| **v0.4.2** | kagome diagnostics stack + exact-reference solver: χ_F = Tr(S)/2 (`nqs_compute_chi_F`), per-bond-class phase probe (`nqs_compute_kagome_bond_phase`), excited-state VMC via Choo–Neupert–Carleo penalty (`nqs_sr_{step,run}_excited`), full-basis Lanczos refinement + multi-Ritz (`nqs_lanczos_{refine,k_lowest}_kagome_heisenberg`). End-to-end research driver (`make research_kagome_N12_diagnostics`). Exact E₀ / E₁ / spin gap Δ anchors on N ≤ 24. 359 / 359 tests passing. |
| **v0.4.3 (last tagged)** | Correctness + capability release on top of v0.4.2: end-to-end audit upgraded four legacy modules (Berry phase / Chern, topological entanglement entropy, Majorana parity + zero-mode detection, NQS optimiser breakdown signalling) from order-of-magnitude analytical proxies to first-principles implementations; MinSR optimiser; full kagome wallpaper-group symmetry projection (`nqs_kagome_{translation,p2,p3,p6,p6m}_perm`); 9-term torque_net with L=2 quadrupolar + time-reversal classification; µMAG-lite trajectory benchmark. |
| **post-v0.4.3 (HEAD, v0.5 candidate)** | Empirical-symbolic kagome AFM Heisenberg research thread.  Sector-projected NQS predictive observables (γ_TEE finite-size flow, S(q,ω), C(d) decay).  Full p6m representation extraction at machine precision (1.835·10⁻¹¹ diagonal, 3.331·10⁻¹⁶ off-diagonal across 432 matrix elements).  Empirical lattice modular S via Zhang-Grover-Vishwanath MES protocol, three runs including doublet-symmetric K=5 with both E₂ partners.  Five independent observables rejecting simple Z₂ TC at N=27. |
| **v0.5 (forthcoming)** | Tier-1 research pillars: NQS, equivariant LLG, learned QEC decoders, neural-operator Boltzmann samplers, Fibonacci-anyon gates. |
