...
| **v0.4.3 (last tagged)** | Correctness + capability release on top of v0.4.2: top-to-bottom audit replaced silently-wrong physics in four legacy modules (Berry phase / Chern, topological entanglement entropy, Majorana parity + zero-mode detection, NQS optimiser breakdown signalling); MinSR optimiser; full kagome wallpaper-group symmetry projection (`nqs_kagome_{translation,p2,p3,p6,p6m}_perm`); 9-term torque_net with L=2 quadrupolar + time-reversal classification; µMAG-lite trajectory benchmark. |
